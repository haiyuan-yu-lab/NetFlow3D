...
if __name__ == "__main__":
	...
	parser.add_argument('-E','--inter_dist_upperlimit', type = float, default = 8.0, help = '[OPTIONAL] Inter-chain distance cutoff. By default 8 angstrom')
	parser.add_argument('-W','--weight_adjustment', type = float, default = 3.0, help = '[OPTIONAL] Edge weight adjustment factor m:  (distance cutoff / actual distance) ** m. By default 3')
	...
	if df.shape[0] == 0:
		...
		sys.exit()
	df["UniProt"] = df.apply(lambda x: funcs.get_uniprot(x, gene2uniprot, ensp2uniprot), axis = 1)
	...
	if df_mis.shape[0] == 0:
		...
		if df.shape[0] == 0:
			...
			for key in uniprot2res:
				function_input.append([key, uniprot2res, output_path + "PDB_graph/", args.PDB_intra_resource, args.PDB_inter_resource, \
			...
			if len(function_input) > 0:
				...
				function_input = [[x, output_path + "PDB_cluster/" + os.path.split(x)[1].split(".")[0] + ".txt", args.intra_dist_upperlimit, args.inter_dist_upperlimit, args.weight_adjustment] for x in set(function_input)]
				p = Pool(args.threads)
			...
			for key in uniprot2res:
				function_input.append([key, uniprot2res, output_path + "AlphaFold2_graph_pLDDT0/", args.AF2_intra_resource, None, \
					args.intra_dist_upperlimit, args.inter_dist_upperlimit])
			p = Pool(args.threads)
			output = p.map(get_graph_by_uniprot_binary_multirun, function_input)
			p.close()
			...
			if len(function_input) > 0:
				...
				function_input = [[x, output_path + "AlphaFold2_cluster/" + os.path.split(x)[1].split(".")[0] + ".txt", args.intra_dist_upperlimit, args.inter_dist_upperlimit, args.weight_adjustment] for x in set(function_input)]
				p = Pool(args.threads)
				output = p.map(get_cluster_from_graph_multirun, function_input)
				p.close()
				df_intra = []
	...
	if df_all.shape[0] == 0:
		...
		df1["Signature_ID"] = df1["Affected_genes"].apply(lambda x: x + "_LoF")
		if df1.shape[0] > 0:
	...
	pd.concat([df_mis_mut, df_lof_mut]).drop(columns = ["UniProt"]).sort_values(by = "Signature_ID").to_csv(args.output_path + args.job_name + "_drivers.txt", sep = "\t", header = True, index = None)
	############################################################################################################################################
