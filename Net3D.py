...
if __name__ == "__main__":
	...
	if df.shape[0] == 0:
		...
		pd.DataFrame({"Signature_ID": [], "Type": [], "Affected_genes": [], "Structure_source": [], "Mutation_frequency": [], "LoF_enrichment": [], "Raw_pvalue": [], "Adjusted_pvalue": []}).to_csv(args.output_path + args.job_name + "_signatures.txt", sep = "\t", header = True, index = None)
		pd.DataFrame({"Hugo_Symbol": [], "Chromosome": [], "Start_Position": [], "End_Position": [], "Reference_Allele": [], "Tumor_Seq_Allele1": [], "Tumor_Seq_Allele2": [], "Variant_Classification": [], "Codons": [], "ENSP": [], "Protein_position": [], "Tumor_Sample_Barcode": [], "UniProt": [], "Signature_ID": []}).to_csv(args.output_path + args.job_name + "_drivers.txt", sep = "\t", header = True, index = None)
		sys.exit()
	############################################################################################################################################
