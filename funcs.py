...
def get_uniprot(row, gene2uniprot, ensp2uniprot):
...
def get_one_delta(G_original, delta_output, restart_prob, max_subnetwork_size, seed):
	# get the original heat distribution
	heat_scores = [info["heat_score"] for u,info in G_original.nodes(data = True)]

	# get a random heat distribution
	G = copy.deepcopy(G_original)
	for u,v in G.edges():
		G[u][v]["weight"] = 1.0
	...
	for i,u in enumerate(node_list):
		G.nodes[u]["heat_score"] = heat_scores[i]
