...
- `{job_name}`_signatures.txt
	...
	6. Mutation_frequency (Format: comma separated `{amino acid residue}:{number of samples}` for spatial clusters of non-truncating mutations; `{gene}:{total number of LoF mutations in all samples}` for protein-specific enrichment of LoF mutations.)
