...
- `{job_name}`_signatures.txt
	...
	6. Mutation_frequency (Format: `{amino acid residue}:{number of samples}` separated by comma for spatial clusters of non-truncating mutations; `{gene}:{number of LoF mutations in all samples}` for protein-specific enrichment of LoF mutations.)
